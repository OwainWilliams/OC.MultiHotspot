...
namespace OC.MultiHotspot.PropertyValueConverter
{
    public class MultiHotspotPropertyValueConverter : IPropertyValueConverter
    {

        private readonly ILogger<MultiHotspotPropertyValueConverter> _logger;
        ...
        public MultiHotspotPropertyValueConverter(ILogger<MultiHotspotPropertyValueConverter> logger)
        {
            _logger = logger;
        }

        public bool IsConverter(IPublishedPropertyType propertyType) =>
            propertyType.EditorUiAlias == "OC.MultiHotspot.PropertyEditorUi";

        public Type GetPropertyValueType(IPublishedPropertyType propertyType)
        {
            return typeof(MultiHotSpot);
        }
        ...
        public bool? IsValue(object? value, PropertyValueLevel level)
        {
            switch (level)
            {
                case PropertyValueLevel.Source:
                    return value != null && value is MultiHotSpot;
                default:
        ...
        )
        {
            if (source?.ToString() is var sourceString && sourceString.IsNullOrWhiteSpace() == false)
            {
                try
                {
                    return JsonSerializer.Deserialize<MultiHotSpot>(sourceString, _options);
                }
                catch (Exception ex)
                {
                    _logger.LogWarning("Failed deserializing HotSpotsEditorValue, {ex}", ex.Message);
                    return null;
                }
            }
